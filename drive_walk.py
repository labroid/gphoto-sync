...
FILE_FIELDS = "id,md5Checksum,mimeType,name,parents,trashed"
...
class GphotoSync:
    ...
    #     # TODO: Think through where to update_start_token after refactor
    ...
    def get_nodes(self):
        cumulative = 0
        nextpagetoken = None
        query = "(mimeType contains 'image/' or mimeType contains 'video/' or mimeType = 'application/vnd.google-apps.folder') and trashed = false"
        while True:
            # nodes = []
            start_time = time.time()
            ...
            cumulative += count
            self.log.info(f"{elapsed:.2} Drive delivered {count} files. Total: {cumulative}")
            sterile_nodes = [self.steralize(x) for x in response['files']]
            nodes = [Gphoto(**x) for x in sterile_nodes]
            self.log.info("Persisting...")
            start_time = time.time()
            Gphoto.objects.insert(nodes)
            elapsed = time.time() - start_time
            self.log.info(f"{elapsed:.2} Persisted")
